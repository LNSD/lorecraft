...
def get_terminal_width() -> int | None:
...
def draw_table(headers: list[str], rows: list[list[str]], terminal_width: int | None = None) -> str:
...
def main() -> None:
    ...
    features_dir = repo_root / 'docs' / 'feat'
    ...
    for md_file in sorted(features_dir.glob('*.md')):
        try:
            ...
            features_by_status[status_key].append({'name': name, 'type': doc_type, 'description': description})
