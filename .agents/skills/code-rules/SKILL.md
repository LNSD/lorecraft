...
`docs/code/` holds the rule documents that govern how code is written in this repository. This skill loads
only their frontmatter, and you choose what to read from it. Selecting well is the whole job: read what the
...
- **Take the most specific match.** A prefix is a group (`principle-*`, `pattern-*`, `python-*`,
  `python-errors-*`, `test-*`). Read the member whose trigger fits — `python-docstrings` for a `"""` block,
  `python-errors-handling` for an `except`, not the group above it. Add a parent only when the task turns on
  what the parent owns: `python-modules` decides where a module lives and what it exports.
...
- **Break ties by specificity:** a package-scoped document (`scope: pkg:<name>`) over a `global` one, then
  `type: core`, then `arch`, then `principle`. Read `type: meta` only when writing a rule document itself —
  those specifications live in `docs/__meta__/`, not in the catalog above.
...
| Use | For |
|---|---|
| `/code-rules-check` | checking a finished changeset against the rules |
| `/code-review` | reviewing a branch for bugs, security and design |
| `/code-format` | running the formatter |
| `/code-check` | running the linter |
| `/code-test` | running tests |
