...
Examples must be idiomatic for this project: Python 3.12, single-quoted strings, `@dataclass` for internal
records, the stdlib `logging` module, Google-style docstrings, and type annotations on every signature.
Examples are fabricated; do not invent tooling the project does not use.
...
Examples show the least invented code that carries the principle, cite no module, and never transcribe real code — no rename anywhere in `src/lorewright/` may be able to falsify the doc ([code](code.md) §6).
