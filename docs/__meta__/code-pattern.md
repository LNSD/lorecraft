...
H1 pattern name, optionally with a parenthetical clarification, e.g. "Registry Pattern (Auto-Discovered Checkers)".

#### Scope line (omitted)

**No scope line.** A pattern document is `scope: "global"` and governs all Python code in the
project, so a bold line saying so restates the frontmatter, the title, and the corpus-wide
...
Patterns documented here MUST already be used in `packages/*/src/` — verify that before writing, then write the example from scratch.
...
Examples are fabricated: show the least invented code that carries the pattern, cite no module, and never transcribe real code — no rename anywhere in `packages/*/src/` may be able to falsify the doc ([code](code.md) §6).
...
Focuses on type safety, failure modes caught early, maintainability, and correctness.
